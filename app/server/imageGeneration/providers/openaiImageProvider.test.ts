...
describe('OpenAI image mapper', () => {
  it('keeps the assembled prompt and maps Visioner size separately', () => {
    ...
    expect(mapOpenAIImageGenerationPayload(request, 'gpt-image-2')).toEqual({
      model: 'gpt-image-2',
  ...
  it('rejects remote references instead of downloading them server-side', () => {
    ...
    expect(() => mapOpenAIImageEditPayload(request, [remote], 'gpt-image-2'))
...
describe('OpenAI image provider', () => {
  it('uses the generations endpoint when there are no references', async () => {
    ...
    const results = await provider.generate(request, [], { providerModel: 'gpt-image-2', signal: new AbortController().signal });

    expect(fetchImplementation).toHaveBeenCalledOnce();
    const [url, init] = fetchImplementation.mock.calls[0] as unknown as [string, RequestInit];
    expect(url).toBe('https://api.openai.com/v1/images/generations');
    expect(JSON.parse(String(init.body))).toMatchObject({ model: 'gpt-image-2', prompt: request.prompt, size: '2048x1152' });
  ...
  it('uses the edits endpoint and repeats image[] in original order', async () => {
    ...
    await provider.generate(request, references, { providerModel: 'gpt-image-2', signal: new AbortController().signal });
  ...
  it('uses the edits endpoint for a single reference', async () => {
    ...
    await provider.generate(request, [fileReference(0)], {
  ...
  it('normalizes missing keys, auth, rate limits, provider failures and invalid images', async () => {
    ...
    await expect(missingKeyProvider.generate(request, [], {
      providerModel: 'gpt-image-2',
      signal: new AbortController().signal,
    ...
    expect(mapOpenAIProviderError(503, {})).toMatchObject({ code: 'PROVIDER_UNAVAILABLE' });
  ...
  it.each([
    ...
    [408, {}, 'TIMEOUT'],
    [400, { error: { param: 'image[]', type: 'invalid_image' } }, 'INVALID_REFERENCE'],
    ...
    expect(logger).toHaveBeenCalledWith('OpenAI image request rejected', expect.objectContaining({ status }));
  });
  ...
  it('propagates AbortSignal and returns a stable cancellation error', async () => {
    ...
    const generation = provider.generate(request, [], { providerModel: 'gpt-image-2', signal: controller.signal });
    controller.abort();
...
describe('image generation model registry and dispatch', () => {
  ...
  it('keeps mock as default and dispatches provider mode through the registry', async () => {
    ...
    expect(generate).toHaveBeenCalledWith(request, [fileReference(0)], expect.objectContaining({ providerModel: 'gpt-image-2' }));
  });
});
